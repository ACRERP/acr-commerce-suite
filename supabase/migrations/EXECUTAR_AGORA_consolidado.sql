-- =====================================================
-- SCRIPT CONSOLIDADO DE MIGRAÇÃO - ACR COMMERCE SUITE
-- Data: 12/12/2025
-- Objetivo: Executar todas as migrações pendentes na ordem correta
-- INSTRUÇÕES: Copie e cole este script no Supabase Dashboard > SQL Editor
-- =====================================================

-- =====================================================
-- MIGRAÇÃO 1: MÓDULO DELIVERY (delivery_men + delivery_orders)
-- Origem: 20251207210000_complete_delivery_module.sql
-- =====================================================
...
-- Expandir delivery_orders (se já existir)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'delivery_orders') THEN
        ...
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS estimated_time INTEGER;
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS preparation_time INTEGER;
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS delivery_time TIMESTAMP WITH TIME ZONE;
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS actual_delivery_time INTEGER;
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS rating INTEGER CHECK (rating BETWEEN 1 AND 5);
        ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS customer_feedback TEXT;
    END IF;
END $$;

-- Habilitar RLS para delivery_men
ALTER TABLE delivery_men ENABLE ROW LEVEL SECURITY;

-- Políticas RLS para delivery_men
DROP POLICY IF EXISTS "delivery_men_read_policy" ON delivery_men;
CREATE POLICY "delivery_men_read_policy" ON delivery_men FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "delivery_men_write_policy" ON delivery_men;
CREATE POLICY "delivery_men_write_policy" ON delivery_men FOR ALL TO authenticated USING (true);

-- =====================================================
-- MIGRAÇÃO 2: MÓDULOS CRM, FISCAL, OS, FINANCEIRO
-- Origem: 20251211000001_apply_all_pending.sql (corrigido)
-- =====================================================
...
-- Expandir products com campos fiscais
DO $$
BEGIN
...
-- POLÍTICAS RLS (nomes únicos)
DROP POLICY IF EXISTS "leads_read_policy" ON leads;
CREATE POLICY "leads_read_policy" ON leads FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "financial_categories_read_policy" ON financial_categories;
CREATE POLICY "financial_categories_read_policy" ON financial_categories FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "cash_flow_read_policy" ON cash_flow;
CREATE POLICY "cash_flow_read_policy" ON cash_flow FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "company_fiscal_settings_read_policy" ON company_fiscal_settings;
CREATE POLICY "company_fiscal_settings_read_policy" ON company_fiscal_settings FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "fiscal_notes_read_policy" ON fiscal_notes;
CREATE POLICY "fiscal_notes_read_policy" ON fiscal_notes FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "service_orders_read_policy" ON service_orders;
CREATE POLICY "service_orders_read_policy" ON service_orders FOR SELECT TO authenticated USING (true);

-- Políticas de escrita
DROP POLICY IF EXISTS "leads_write_policy" ON leads;
CREATE POLICY "leads_write_policy" ON leads FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "cash_flow_write_policy" ON cash_flow;
CREATE POLICY "cash_flow_write_policy" ON cash_flow FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "service_orders_write_policy" ON service_orders;
CREATE POLICY "service_orders_write_policy" ON service_orders FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "service_order_services_write_policy" ON service_order_services;
CREATE POLICY "service_order_services_write_policy" ON service_order_services FOR ALL TO authenticated USING (true);

DROP POLICY IF EXISTS "service_order_parts_write_policy" ON service_order_parts;
CREATE POLICY "service_order_parts_write_policy" ON service_order_parts FOR ALL TO authenticated USING (true);

-- =====================================================
-- MIGRAÇÃO 3: PERMISSÕES DELIVERY
-- Origem: 20251211000003_fix_delivery_grants.sql
-- =====================================================

GRANT ALL ON TABLE delivery_men TO anon, authenticated, service_role;
GRANT ALL ON TABLE delivery_orders TO anon, authenticated, service_role;
...
SELECT 
    'delivery_men' as tabela,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'delivery_men') 
        THEN '✅ Criada' 
        ELSE '❌ Não existe' 
    END as status
UNION ALL
...
SELECT 
    'cash_flow' as tabela,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cash_flow') 
        THEN '✅ Criada' 
        ELSE '❌ Não existe' 
    END as status
UNION ALL
SELECT 
    ...
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'service_orders') 
        THEN '✅ Criada' 
        ELSE '❌ Não existe' 
    END as status;
