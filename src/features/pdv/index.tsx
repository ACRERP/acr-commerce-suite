...
export function PDVFeature() {
    ...
    const handleFinalizeSale = (payments: any[], deliveryData?: any) => {
        if (!cashRegister) {
        ...
        createSaleMutation.mutate({
            ...
            deliveryData,
            saleType: cart.deliveryFee > 0 ? 'delivery' : 'counter',
        }, {
            onSuccess: (data) => {
                // Prepare receipt data BEFORE clearing cart
                const receipt: ReceiptData = {
                    saleId: data.id,
                    companyName: "ACR ERP", // Todo: Get from config
                    operatorName: cashRegister.operator_name,
                    cashRegisterId: cashRegister.id,
                    ...
                    payments: payments.map(p => ({
                        method: p.payment_method, // Using corrected property
                        amount: p.amount,
                ...
                // Handle table checkout if applicable
                if (cart.tableId && cart.tableOrderId) {
                    gastronomyService.finalizeCheckout(cart.tableId, cart.tableOrderId)
                        .catch(err => console.error("Error finalizing table:", err));
                }

                // Clear cart only after modal is ready
                cart.clearCart();
            },
            onError: (error) => {
    ...
    return (
        <div className="h-screen flex flex-col bg-neutral-100 overflow-hidden font-sans">
                        ...
                        <div className="space-y-1">
                            ...
                            <Popover open={openClientCombo} onOpenChange={setOpenClientCombo}>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        {cart.clientId
                                            ? clients?.find((client) => client.id.toString() === cart.clientId)?.name || cart.clientName
                                            : "Selecionar Cliente..."}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-56">
                                    ...
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem onClick={() => setShowCashOpsModal(true)}>
                                        <DollarSign className="mr-2 h-4 w-4" />
                                        <span>Sangria / Suprimento</span>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => setShowRecentSalesModal(true)}>
                                        <History className="mr-2 h-4 w-4" />
                                        <span>Últimas Vendas / Estorno</span>
                                    </DropdownMenuItem>
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-50" onClick={() => setShowCloseCashModal(true)}>
            ...
            <PaymentModal
                ...
                isDelivery={cart.deliveryFee > 0}
                defaultCustomerName={cart.clientName || ''}
            />
            ...
            {cashRegister && (
                <>
                    <CashOperationsModal
                        open={showCashOpsModal}
                        onOpenChange={setShowCashOpsModal}
                    ...
                    />
                    <RecentSalesModal
                        open={showRecentSalesModal}
                        onOpenChange={setShowRecentSalesModal}
                        registerId={cashRegister.id}
                    />
                </>
            )}
