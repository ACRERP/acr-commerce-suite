...
export class DeliveryService {
  ...
  /**
   * Atualizar status de forma inteligente (chamando RPCs específicas se necessário)
   */
  async updateDeliveryStatus(id: number, status: string): Promise<void> {
    switch (status) {
        case 'in_transit':
            await this.startDelivery(id);
            break;
        case 'delivered':
            await this.completeDelivery(id);
            break;
        case 'cancelled':
            await this.cancelDelivery(id, 'Cancelado via Kanban');
            break;
        default:
            await this.updateDelivery(id, { status: status as any });
    }
  }
  ...
  async updateRegion(id: number, updates: any) {
    const { data, error } = await supabase
      .from('delivery_regions')
      .update(updates)
      .eq('id', id)
  ...
  /**
   * Configurações de Entrega (Google Maps)
   */
  async getSettings() {
    const { data, error } = await supabase
    ...
    if (error) {
      // Se não existir, retorna padrão (fallback de segurança)
      return { price_per_km: 1.50, calculation_mode: 'hybrid', google_maps_key: '' };
    }
    return data;
  }
  ...
  /**
   * Gerar link do WhatsApp para notificação
   */
  getWhatsAppLink(phone: string, message: string): string {
    const cleaned = phone.replace(/\D/g, '');
    const encodedMsg = encodeURIComponent(message);
    return `https://wa.me/55${cleaned}?text=${encodedMsg}`;
  }
}
