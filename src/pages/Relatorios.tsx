...
import { ChurnReportDialog } from "@/components/reports/ChurnReportDialog";
...
const Relatorios = () => {
  ...
  const handleReportAction = (reportTitle: string) => {
    if (reportTitle === "Análise de Churn") {
      setIsChurnOpen(true);
    } else {
      // Placeholder for other reports
      console.log(`Action for ${reportTitle}`);
    }
  };
    ...
    <MainLayout>
      <div className="container-premium py-8 space-y-8 animate-fade-in-up">
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          <TabsList className="bg-black/5 dark:bg-white/5 p-1 rounded-xl w-full md:w-auto">
            <TabsTrigger value="overview" className="rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-neutral-800 data-[state=active]:shadow-sm px-6">Visão Geral (BI)</TabsTrigger>
            <TabsTrigger value="dre" className="rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-neutral-800 data-[state=active]:shadow-sm px-6">DRE Gerencial</TabsTrigger>
          </TabsList>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {detailedReports.map((report, index) => {
                  ...
                  return (
                    <div key={index}
                      className="card-premium p-6 hover-lift group cursor-pointer border border-transparent hover:border-primary-100 dark:hover:border-primary-900/50"
                      onClick={() => handleReportAction(report.title)}
                    >
                      ...
                      <Button variant="outline" className="w-full justify-between group-hover:bg-primary-50 group-hover:text-primary-700 dark:group-hover:bg-neutral-800 group-hover:border-primary-200 transition-colors">
                        {report.title === "Análise de Churn" ? "Visualizar Risco" : "Baixar Arquivo"}
                        {report.title === "Análise de Churn" ? <Users className="w-4 h-4 ml-2" /> : <Download className="w-4 h-4 ml-2" />}
                      </Button>
                    </div>
                  );
                })}
          ...
          <TabsContent value="dre" className="animate-in fade-in slide-in-from-right-4">
            <DRETab />
          </TabsContent>
        </Tabs>
      </div>

      <ChurnReportDialog open={isChurnOpen} onOpenChange={setIsChurnOpen} />
    </MainLayout>
  );
};
