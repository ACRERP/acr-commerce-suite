...
import { UpdateClientDialog } from "@/components/clients/UpdateClientDialog";
  ...
  return (
    <MainLayout>
      <div className="container-premium py-8 space-y-8 animate-fade-in-up">
        ...
        <BulkClientImport open={isImportOpen} onOpenChange={setIsImportOpen} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="relative z-10 flex items-start justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">
                  {clientsData ? clientsData.length : <Loader2 className="w-6 h-6 animate-spin" />}
                </h3>
              </div>
          ...
          <div className="card-premium hover-lift group relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-start justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">
                  {clientsData
                    ? clientsData.filter(c => new Date(c.created_at || '').getMonth() === new Date().getMonth()).length
          ...
          <div className="card-premium hover-lift group relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-start justify-between">
              <div>
                <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400 mb-1">Com Compras</p>
                <h3 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">
                  {clientsData
                    ? clientsData.filter(c => (c.total_purchases || 0) > 0).length
                    : <Loader2 className="w-6 h-6 animate-spin" />}
                </h3>
              </div>
                ...
                <h3 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">
                  {clientsData && clientsData.length > 0
                    ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
                      clientsData.reduce((acc, c) => acc + (c.total_spent || 0), 0) / clientsData.length
                    )
                    : 'R$ 0,00'}
                </h3>
              </div>
        ...
        <div className="card-premium p-0 overflow-hidden">
                        ...
                        <td className="py-4 px-6 text-center">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => handleEditClick(client)}>Editar cliente</DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-red-600 focus:text-red-600" onClick={() => handleDeleteClick(client)}>
                                Excluir cliente
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
          ...
          <div className="p-4 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex items-center justify-between">
            <span className="text-sm text-neutral-500">
              {clientsData ? `Mostrando ${clientsData.length} clientes` : 'Opções de visualização'}
            </span>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled className="h-8">Anterior</Button>
              <Button variant="outline" size="sm" disabled className="h-8">Próximo</Button>
            </div>
          </div>
        </div>
      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br /><br />
              Essa ação não pode ser desfeita e removerá o histórico de compras deste cliente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} className="bg-red-600 hover:bg-red-700">
              {deleteClientMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              Excluir
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
