...
import {
  ...
  updateTransaction,
  deleteTransaction,
  CreateFinancialTransactionData as CreateTransactionData,
...
import { BankingReconciliation } from "@/components/dashboard/finance/BankingReconciliation";
...
const Financeiro = () => {
  ...
  const handleConfirmDelete = () => {
    if (transactionToDelete) {
      deleteMutation.mutate(transactionToDelete.id as any);
    }
  };

  const handleFormSubmit = async (data: CreateTransactionData) => {
    if (selectedTransaction) {
      await updateMutation.mutateAsync({ id: selectedTransaction.id as any, data });
    } else {
  ...
  const getAnomalyForTransaction = (id: string | number) => anomalies.find(a => a.transactionId === id);
      ...
      <div className="container-premium py-8 space-y-8 animate-fade-in-up">
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          <TabsList className="bg-black/5 dark:bg-white/5 p-1 rounded-xl w-full md:w-auto">
            <TabsTrigger value="overview" className="rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-neutral-800 data-[state=active]:shadow-sm px-6">Visão Geral</TabsTrigger>
            <TabsTrigger value="reconciliation" className="rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-neutral-800 data-[state=active]:shadow-sm px-6">Conciliação Bancária</TabsTrigger>
          </TabsList>
                        ...
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-xs text-neutral-500">
                              ...
                              <span className="capitalize">{(tx as any).category_name || 'Geral'}</span>
                              {getAnomalyForTransaction(tx.id as any) && (
                                <>
                                  <span className="w-1 h-1 rounded-full bg-neutral-300"></span>
          ...
          <TabsContent value="reconciliation" className="animate-in fade-in slide-in-from-right-4">
