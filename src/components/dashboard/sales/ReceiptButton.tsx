...
// Helper to keep logic clean, but now accepts config
function convertToReceiptData(sale: Sale, items: SaleItem[], payments: SalePayment[], companyConfig?: any): ReceiptData {
  return {
    saleId: typeof sale.id === 'string' ? parseInt(sale.id) : sale.id,
    companyName: companyConfig?.razao_social || 'ACR Store',
    ...
    cashRegisterId: 1, // Generic for Dashboard Sales (or fetch if connected to one)
...
export function ReceiptButton({
  ...
  const receiptData = React.useMemo(() =>
    convertToReceiptData(sale, items, payments, companyConfig),
    [sale, items, payments, companyConfig]
  );
