...
const quickLinks = [
    { id: "sales", label: "Nova Venda", icon: ShoppingCart, color: "bg-purple-600", path: "/vendas", moduleId: 'sales' },
    { id: "os", label: "Nova OS", icon: Wrench, color: "bg-orange-600", path: "/os", moduleId: 'service_orders' },
    { id: "client", label: "Novo Cliente", icon: Users, color: "bg-green-600", path: "/clientes/novo", moduleId: 'clients' },
    { id: "product", label: "Novo Produto", icon: Package, color: "bg-blue-600", path: "/produtos/novo", moduleId: 'inventory' },
];
...
export function AppGrid() {
    ...
    const { data: metrics } = useQuery({
        ...
        queryFn: async () => {
            ...
            const lowStock = products?.filter(p => p.stock_quantity > 0 && p.stock_quantity <= (p.minimum_stock_level || 5)).length || 0;
            ...
            const overdue = transactions?.filter(t =>
                t.status === 'pending' && new Date(t.due_date) < today
            ).length || 0;
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {activeProfile?.modules?.includes('sales') && (
                ...
                {activeProfile?.modules?.includes('service_orders') && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:col-span-3">
                    {activeProfile?.modules?.includes('inventory') && (
                    ...
                    {activeProfile?.modules?.includes('delivery') && (
                    ...
                    {activeProfile?.modules?.includes('finance') && (
                        <Card className="hover:border-red-200 transition-colors border-l-4 border-l-red-500 shadow-sm">
                            <CardContent className="p-4 flex items-center justify-between">
                                <div>
                                    <p className="text-xs text-muted-foreground font-semibold uppercase">Contas Atrasadas</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {quickLinks
                    .filter(link => !link.moduleId || activeProfile?.modules?.includes(link.moduleId))
                    .map((link) => (
                        <Link
                            key={link.id}
                            to={link.path}
